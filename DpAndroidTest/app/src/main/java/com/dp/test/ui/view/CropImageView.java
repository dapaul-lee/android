...
/**
 * Created by dapaul on 2017/5/19.
 */

public class CropImageView extends ImageView implements ScaleGestureDetector.OnScaleGestureListener, View.OnTouchListener {
    ...
    public CropImageView(Context context, AttributeSet attrs) {
                ...
                new GestureDetector.SimpleOnGestureListener() {
                    ...
                    public boolean onDoubleTap(MotionEvent e) {
                        ...
                        if (getScale() < mScaleMin) {
                            postDelayed(new AutoScaleRunnable(mScaleMin, x, y), 16);
                        } else {
        ...
        mAspectX = ta.getInteger(com.githang.clipimage.R.styleable.ClipImageView_civWidth, 1);
        mAspectY = ta.getInteger(com.githang.clipimage.R.styleable.ClipImageView_civHeight, 1);
        ...
        mDrawCircleFlag = ta.getBoolean(com.githang.clipimage.R.styleable.ClipImageView_civClipCircle, false);
        mRoundCorner = ta.getDimension(com.githang.clipimage.R.styleable.ClipImageView_civClipRoundCorner, 0);
...
//            final Bitmap source = decoder.decodeRegion(clipRect, ops);
            ...
            return Bitmap.createBitmap(mBitmapSource, 0, 0, mBitmapSource.getWidth(), mBitmapSource.getHeight(), outputMatrix, false);
        } catch (Exception e) {
            return clip();
        } finally {
    ...
    private void updateBorder() {
        ...
        if (mDrawCircleFlag == true) { // 如果是圆形,宽高比例是1:1
            ...
            //Clip border is the same with view border
            mClipBorder.top = mClipPadding;//(height - borderHeight) / 2;
            mClipBorder.bottom = height - mClipPadding; // mClipBorder.top + borderHeight;
        }
    }
    ...
    public void setImageResource(int resId) {
        ...
        //Get bitmap
        mBitmapSource = BitmapFactory.decodeResource(getResources(), R.drawable.pic2);
        postResetImageMatrix();
    }
...
//                mSampleSize = findBestSample(options.outWidth, getClipBorder().width());
//
...
//                Bitmap target;
//                if (mDegree == 0) {
//                    target = mBitmapSource;
//                } else {
//                    final Matrix matrix = new Matrix();
//                    matrix.postRotate(mDegree);
//                    target = Bitmap.createBitmap(mBitmapSource, 0, 0, mBitmapSource.getWidth(), mBitmapSource.getHeight(), matrix, false);
//                    if (target != mBitmapSource && !mBitmapSource.isRecycled()) {
